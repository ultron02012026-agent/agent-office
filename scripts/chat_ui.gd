...
func _request_greeting(room_name: String):
	...
	if _gateway_ws and _gateway_ws.is_ws_connected():
		_gateway_ws.send_message(room_name, "Ethan just walked into your office in Agent Office. Give a brief greeting (1 sentence).")
	else:
		# Fallback to HTTP
		var messages = [
			{"role": "system", "content": _build_system_prompt()},
			{"role": "user", "content": "Ethan just walked into your office in Agent Office. Give a brief greeting (1 sentence)."}
		]
